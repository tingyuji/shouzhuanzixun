...
(function(){
	...
	function pcc_online() {
		if (! /(^|; )u=[^;]/.test(document.cookie)) {
			...
			if (uf=="") {
				...
				if (now - uf > 43200000) {
					calljs("http://ivy.pconline.com.cn/adpuba/online-self.jsp");
				}
			}
		} else {
	...
	function pcc_u() {
		if (! /(^|; )u=[^;]/.test(document.cookie)) {
			calljs("http://ivy.pconline.com.cn/adpuba/online-u.jsp");
		}
	}
	function pcc_uf() {
		var uf=document.cookie.replace(/(^|.*; )uf=([^;]+).*|.*/,"$2")
		if (uf=="") {
			...
			document.cookie = "uf=" + ((new Date().getTime())) + ";domain=" +
				top.location.href.replace(/http:\/\/.+?\.(.*?)\/.*/, '.$1') +
				";path=/;expires=Mon, 30-Nov-2099 08:00:00 GMT;";
			} catch(e) {}
		} else {
			var now = new Date().getTime();
			if (now - uf > 43200000) {
				calljs("http://ivy.pconline.com.cn/adpuba/online-uf3.jsp");
			}
		}
	}
	...
	try{
    	if (top.location.href.replace(/http:\/\/.+?\.(.*?).com.cn\/.*/, '$1') == 'pconline') { 
    		pcc_online();
		} else {
			pcc_u();
			}
    	...
    	if (! /(^|; )c=[^;]/.test(document.cookie)) {
	    	if (top.location.href.replace(/http:\/\/.+?\.(.*?).com.cn\/.*/, '$1') == 'pconline') {
	    		calljs("http://ivy.pconline.com.cn/adpuba/online-c-self.jsp");
			} else {
				calljs("http://ivy.pconline.com.cn/adpuba/online-c.jsp");
				}
    	}
    	// 测试弹出用的 要延时才正确
